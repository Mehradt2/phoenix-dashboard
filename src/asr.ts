...
export async function modelStatus(mode:ModelMode=getMode()){
 ...
 return{mode,hardware:h,plan,modelProfile:RUNTIME.modelProfile,modelRuntime:resolveModelRuntime(),cacheEnabled:Boolean('caches'in globalThis),policy:'browser-asr-policy-1.2.0',timeouts:{modelLoadMs:MODEL_LOAD_TIMEOUT_MS},fallbackModel:BASE_FALLBACK_ID};
}
...
export async function warmModel(mode:ModelMode,progress:Progress){
 const h=await probeHardware(),p=choosePlan(mode,h,RUNTIME.modelProfile),k=RUNTIME.modelProfile+'|'+p.id+'|'+p.device+'|'+JSON.stringify(p.dtype);
